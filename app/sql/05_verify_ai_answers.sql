...
-- -----------------------------------------------------------------------------
-- 検算4: AIの理由が断定表現になっていないか
--   期待: 0行。理由は「仮説」として画面に出すので、購買から属性を断定する
--         書き方になっていないことを確認する。実測では0行だった。
--   これは漏洩チェックではない。正解を渡していないことの保証は 04 のプロンプト
--   定義そのもの（属性を一切入れていない）であって、出力の検査では担保できない。
-- -----------------------------------------------------------------------------
SELECT
